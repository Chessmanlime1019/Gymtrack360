  ...
  return (
    <div className="relative flex-1 max-w-md" ref={ref}>
      ...
      <input
        ...
        disabled={!soportado}
        placeholder={soportado ? "Buscar clientes..." : "B̼squeda no disponible para tu rol"}
        className="w-full rounded-lg bg-background border border-line pl-9 pr-8 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary disabled:opacity-40 disabled:cursor-not-allowed"
      />
      {cargando && (
      ...
      {abierto && soportado && termino.trim().length >= 2 && (
        <div className="absolute left-0 right-0 mt-2 bg-surface border border-line rounded-xl shadow-lg z-50 overflow-hidden max-h-72 overflow-y-auto">
          {resultados.length === 0 && !cargando ? (
            ...
            resultados.map((r) => (
              <button
                ...
                className="w-full text-left px-4 py-2.5 hover:bg-black/5 transition-colors border-b border-line last:border-0"
              >
